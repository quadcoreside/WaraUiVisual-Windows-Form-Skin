...
using WaraUi.Controls;
using WaraUi.Properties;

namespace WaraUi
{
    public class WaraSkinManager
    {
        //Singleton instance
        private static WaraSkinManager instance;

        //Forms to control
        private readonly List<WaraForm> formsToManage = new List<WaraForm>();
        ...
        public static WaraSkinManager Instance
        {
            get { return instance ?? (instance = new WaraSkinManager()); }
        }

        public void AddFormToManage(WaraForm materialForm)
        {
            formsToManage.Add(materialForm);
            UpdateBackgrounds();
        }

        public void RemoveFormToManage(WaraForm materialForm)
        {
        ...
        private void UpdateControl(Control controlToUpdate, Color newBackColor)
        {
            ...
            var tabControl = controlToUpdate as WaraTabControl;
            if (tabControl != null)
            {
            ...
            if (controlToUpdate is WaraDivider)
            {
                controlToUpdate.BackColor = GetDividersColor();
            }

	        if (controlToUpdate is WaraListView)
	        {
		        controlToUpdate.BackColor = newBackColor;
	        }

            if (controlToUpdate is ListView)
            {
                controlToUpdate.BackColor = newBackColor;
                controlToUpdate.ForeColor = GetPrimaryTextColor();

                ListView lv = (ListView)controlToUpdate;
                foreach(ListViewItem lvi in lv.Items)
                {
                    lvi.BackColor = newBackColor;
                    lvi.ForeColor = GetPrimaryTextColor();
                }
            }

            if (controlToUpdate is Panel)
            {
                controlToUpdate.ForeColor = GetPrimaryTextColor();
            }

            if (controlToUpdate is Label)
            {
                controlToUpdate.ForeColor = GetPrimaryTextColor();
            }

            if (controlToUpdate is NumericUpDown)
            {
                controlToUpdate.BackColor = newBackColor;
                controlToUpdate.ForeColor = GetPrimaryTextColor();
            }

            if (controlToUpdate is GroupBox)
            {
                controlToUpdate.BackColor = newBackColor;
                controlToUpdate.ForeColor = GetPrimaryTextColor();
            }

            if (controlToUpdate is WaraPanel)
            {
                controlToUpdate.BackColor = newBackColor;
            }
